...
def get_all_indian_stocks():
    ...
    # 2. Try to get BSE stocks
    try:
        # We'll use a web scraping approach for BSE
        # This uses an unofficial API endpoint that lists BSE stocks
        headers = {
    ...
    # 3. If both fail, fallback to Money Control scraping


    # 4. Last resort fallback
    if not all_symbols:
        print("All methods failed, using hardcoded list of major Indian stocks")
        # Include a larger fallback list of major stocks from both exchanges
        major_stocks = [
            # NSE
...
def get_financial_data(symbols):
    ...
    for i, symbol in enumerate(symbols):
        try:
            ...
            # Skip stocks with insufficient financial data
            ...
            try:
                ...
                try:
                    # First try to get ROE directly from info
                    if 'returnOnEquity' in info:
                        roe = info['returnOnEquity'] * 100  # Convert to percentage
                    else:
                        # Fallback to calculation if not available in info
                ...
                # Debt to Equity
                try:
                    # First try to get debt-to-equity directly from info
                    if 'debtToEquity' in info:
                        debt_to_equity = info['debtToEquity'] / 100  # Convert to decimal ratio
                    else:
                        # Fallback to calculation if not available in info
                        total_debt = balance_sheet.loc['Total Debt'].iloc[
                            0] if 'Total Debt' in balance_sheet.index else None
                        if total_debt is None:
                            # Try to calculate from Long Term Debt + Current Debt
                ...
                try:
                    ...
                    try:
                        # Try alternative approach using EPS data or annual reports
                        ...
                        if eps_key and 'previousClose' in info and info['previousClose'] > 0:
                            ...
                            if pe_ratio:
                                ...
                                five_year_growth = info.get('fiveYearAvgDividendYield',
                                                            10)  # default to 10% if not available
                ...
                try:
                    if fcf and fcf > 0:
                        # Use either analyst growth estimates or historical data
                        growth_rate = info.get('earningsGrowth', 0.10)  # default to 10% if not available

                        # Cap growth rate at reasonable levels (5-20%)
                        growth_rate = max(0.05, min(growth_rate, 0.20))

                        discount_rate = 0.12  # 12% discount rate (Buffett's approximate hurdle rate)
                        ...
                        # Terminal value with conservative growth rate (3%)
